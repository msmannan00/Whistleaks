...
import com.hiddenservices.dtechservices.BuildConfig;
...
import com.hiddenservices.dtechservices.constants.*;
...
import com.hiddenservices.dtechservices.libs.views.ColorAnimator;
...
import static com.hiddenservices.dtechservices.constants.constants.CONST_GENESIS_DOMAIN_URL;
...
import static com.hiddenservices.dtechservices.constants.status.sSettingLanguage;
...
public class homeViewController {
    ...
    public void initialization(eventObserver.eventListener event, AppCompatActivity context, Button mNewTab, ConstraintLayout webviewContainer, TextView loadingText, ProgressBar progressBar, editTextManager searchbar, ConstraintLayout splashScreen, ImageView loading, View banner_ads, ImageButton gateway_splash, LinearLayout top_bar, GeckoView gecko_view, ImageView backsplash, Button connect_button, View pFindBar, EditText pFindText, TextView pFindCount, androidx.constraintlayout.widget.ConstraintLayout pTopLayout, ImageButton pVoiceInput, ImageButton pMenu, androidx.core.widget.NestedScrollView pNestedScroll, ImageView pBlocker, ImageView pBlockerFullSceen, View mSearchEngineBar, TextView pCopyright, RecyclerView pHistListView, com.google.android.material.appbar.AppBarLayout pAppBar, ImageButton pOrbotLogManager, ConstraintLayout pInfoLandscape, ConstraintLayout pInfoPortrait, FragmentContainerView pTabFragment, LinearLayout pTopBarContainer, ImageView pSearchLock, ImageView pTopBarHider, ImageView pNewTabBlocker, CoordinatorLayout mCoordinatorLayout, ImageView pImageDivider, ImageButton pPanicButton, ImageView pGenesisLogo, ImageButton pPanicButtonLandscape, ImageView pTorDisabled, LinearLayout pTopBarContainerWhistle, ImageButton pWhisperTor, ImageButton pWhisperIdentity) {
        ...
        this.mWhisperTor = pWhisperTor;
        this.mWhisperIdentity = pWhisperIdentity;
    ...
    public void initializeViews() {
        ...
        if(!status.sDefaultTor){
            mWhisperTor.setColorFilter(ContextCompat.getColor(mContext, R.color.c_holo_gray));
            mWhisperIdentity.setColorFilter(ContextCompat.getColor(mContext, R.color.c_holo_gray));
            mWhisperIdentity.setEnabled(false);
        }else {
            mWhisperTor.setColorFilter(ContextCompat.getColor(mContext, R.color.c_icon_tint));
            mWhisperIdentity.setColorFilter(ContextCompat.getColor(mContext, R.color.c_icon_tint));
            mWhisperIdentity.setEnabled(true);
        }
    ...
    void initTab(int count, boolean pForced, homeEnums.eHomeViewCallback pEvent, List<Object> pData) {
        ...
        String mText = count + strings.GENERIC_EMPTY_STR;
        mNewTab.setText(mText);
    ...
    void onOpenMenu(View view, boolean canGoForward, boolean isLoading, int userAgent, String mURL, boolean pIsBookmarked) {
        ...
        LinearLayout loginPage = popupView.findViewById(R.id.menu31);
        ...
        if(status.mWhistleMode && !status.sBusinessCode.contains(".")){
        ...
        if(status.mWhistleMode){
            popupView.findViewById(R.id.menu11).setVisibility(View.GONE);
            popupView.findViewById(R.id.menu7).setVisibility(View.GONE);
            popupView.findViewById(R.id.pMenuOpenRecentTab).setVisibility(View.GONE);
            popupView.findViewById(R.id.pMenuOpenNewTab).setVisibility(View.GONE);
    ...
    public void onToggleWhistleMode(){
        if(status.mWhistleMode){
            mNewTab.setEnabled(false);
            mNewTab.setVisibility(View.GONE);
            mTopBarContainerWhistle.setVisibility(View.VISIBLE);
            mTopBarContainerWhistle.animate().alpha(1).setDuration(200).withEndAction(() -> {
            });
            mTopBarContainerWhistle.bringToFront();
        }else {
            mNewTab.setEnabled(true);
            mNewTab.setVisibility(View.VISIBLE);
            mTopBarContainerWhistle.animate().alpha(0).setDuration(200).withEndAction(() -> {
                mTopBarContainerWhistle.setVisibility(View.GONE);
            });
            mTopBarContainer.bringToFront();
        }
    }
    ...
    void downloadNotification(String message) {
    ...
    public void onUpdateFindBarCount(int index, int total) {
        if (total == 0) {
            ...
            String mText = (total + "/" + index);
            mFindCount.setText(mText);
    ...
    public void triggerUpdateSearchBar(String url, boolean showProtocol, boolean pClearText, boolean pForced) {
        ...
        mSearchUpdateHandler.postDelayed(() ->
        {
            ...
            if (mSearchbar.getText().toString().equals("loading")) {
                String mText = "about:blank";
                mSearchbar.setText(mText);
            }
        }, mDelay);
    }
    ...
    void onProgressBarUpdate(int value, boolean isStaticURL) {

        if(status.sProxyChanging){
            return;
        }
                ...
                if (msg.what == messages.MESSAGE_ON_URL_LOAD) {
                    if (mEvent.invokeObserver(null, homeEnums.eHomeViewCallback.M_HOME_PAGE) == null) {
                        ...
                        if (status.sSettingRedirectStatus.equals(strings.GENERIC_EMPTY_STR)) {
                            ...
                            mEvent.invokeObserver(Collections.singletonList(helperMethod.getDomainName(status.sSettingDefaultSearchEngine)), homeEnums.eHomeViewCallback.ON_URL_LOAD_START);
                        } else {
                            mEvent.invokeObserver(Collections.singletonList(helperMethod.getDomainName(status.sSettingRedirectStatus)), homeEnums.eHomeViewCallback.ON_URL_LOAD_START);
                        }
                    }
