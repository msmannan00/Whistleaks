...
import static com.hiddenservices.dtechservices.pluginManager.pluginEnums.eMessageManager.M_LONG_PRESS_URL;
import static com.hiddenservices.dtechservices.pluginManager.pluginEnums.eMessageManager.M_LONG_PRESS_WITH_LINK;
...
import com.hiddenservices.dtechservices.appManager.homeManager.geckoManager.downloadManager.geckoDownloadManager;
    ...
    public void onContextMenu(@NonNull GeckoSession var1, int var2, int var3, @NonNull GeckoSession.ContentDelegate.ContextElement var4) {
        ...
        if (var4.type != 0 && var4.srcUri != null) {
            if (var4.linkUri != null) {
                mEvent.invokeObserver(Arrays.asList(var4.linkUri, mGeckoDataModel.mSessionID, var4.srcUri, title, mGeckoDataModel.mTheme, var4.altText, mGeckoSession, status.mWhistleMode ,mContext.get()), M_LONG_PRESS_WITH_LINK);
            } else {
                try {
                    ...
                    mEvent.invokeObserver(Arrays.asList(var4.srcUri, mGeckoDataModel.mSessionID, mTitle, mGeckoDataModel.mTheme, mGeckoSession, status.mWhistleMode, mContext.get()), homeEnums.eGeckoCallback.ON_LONG_PRESS);
                } catch (Exception ex) {
                    ex.printStackTrace();
                    Log.i("", "");
                }
            ...
            mEvent.invokeObserver(Arrays.asList(var4.linkUri, mGeckoDataModel.mSessionID, title, mGeckoDataModel.mTheme, mGeckoSession, status.mWhistleMode ,mContext.get()), M_LONG_PRESS_URL);
        }
    }
