package com.hiddenservices.dtechservices.pluginManager.messagePluginManager;
...
import static com.hiddenservices.dtechservices.constants.strings.BRIDGE_CUSTOM_INVALID_TYPE;
import static com.hiddenservices.dtechservices.constants.strings.BRIDGE_CUSTOM_INVALID_TYPE_STRING;
import static com.hiddenservices.dtechservices.constants.strings.GENERIC_EMPTY_DOT;
import static com.hiddenservices.dtechservices.constants.strings.MESSAGE_NOT_SECURE_HTTPS_SERVICE;
import static com.hiddenservices.dtechservices.constants.strings.MESSAGE_PLAYSTORE_NOT_FOUND;
import static com.hiddenservices.dtechservices.constants.strings.MESSAGE_SECURE_ONION_SERVICE;
import static com.hiddenservices.dtechservices.pluginManager.pluginEnums.eMessageManager.*;
import static com.hiddenservices.dtechservices.pluginManager.pluginEnums.eMessageManagerCallbacks.*;
import static com.hiddenservices.dtechservices.pluginManager.pluginEnums.eMessageManagerCallbacks.M_CLEAR_BOOKMARK;
import static com.hiddenservices.dtechservices.pluginManager.pluginEnums.eMessageManagerCallbacks.M_CLEAR_HISTORY;
import static com.hiddenservices.dtechservices.pluginManager.pluginEnums.eMessageManagerCallbacks.M_LOAD_NEW_TAB;
...
public class messageManager implements View.OnClickListener, DialogInterface.OnDismissListener {
    ...
    private void businessCode() {
        initializeDialog(R.layout.popup_business_id, Gravity.CENTER);
        mDialog.setCancelable(false);
        mDialog.getWindow().setLayout(ConstraintLayout.LayoutParams.MATCH_PARENT, ConstraintLayout.LayoutParams.MATCH_PARENT);
        mDialog.getWindow().getAttributes().windowAnimations = R.style.WindowAnimationFadeInOut;

        ColorDrawable back = new ColorDrawable(Color.TRANSPARENT);
        InsetDrawable inset = new InsetDrawable(back, helperMethod.pxFromDp(0), 0, helperMethod.pxFromDp(0), 0);
        mDialog.getWindow().setBackgroundDrawable(inset);

        Button mPopupBusinessCodeNext = mDialog.findViewById(R.id.pPopupBuinessCodeNext);
        mDialog.setOnShowListener(dialog -> mContext.getWindow().setSoftInputMode(WindowManager.LayoutParams.SOFT_INPUT_ADJUST_NOTHING));
        mPopupBusinessCodeNext.setOnClickListener(this);
    }
    ...
    private void downloadFileLongPress() {
        ...
        boolean mWhistleMode = (boolean)mData.get(5);
        if (title.length() > 0) {
            title = title + " | ";
        }
        ...
        if(mWhistleMode){
            mPopupLongPressOptionNewTab.setVisibility(View.GONE);
            mDialog.findViewById(R.id.inflatableDivider1).setVisibility(View.GONE);
        }
    ...
    private void openURLLongPress() {
        ...
        if(mWhistleMode){
            mPopupURLLongPressNewTab.setVisibility(View.GONE);
            mDialog.findViewById(R.id.inflatableDivider1).setVisibility(View.GONE);
        }
    ...
    private void popupDownloadFull() {
        ...
        boolean mWhistleMode = (boolean)mData.get(7);
        ...
        if(mWhistleMode){
            mPopupDownloadFullNewTab.setVisibility(View.GONE);
            mPopupDownloadFullImageNewTab.setVisibility(View.GONE);

            mDialog.findViewById(R.id.inflatableDivider1).setVisibility(View.GONE);
            mDialog.findViewById(R.id.inflatableDivider2).setVisibility(View.GONE);
        }
    ...
    public void onClick(View view) {
        if (view.getId() == R.id.pPopupToastTrigger ||
            ...
            onDismiss();
        } else if (view.getId() == R.id.pPopupBuinessCodeNext) {
            TextView mPopupBuinessCodeError = mDialog.findViewById(R.id.pPopupBuinessCodeError);
            EditText mPopupBuinessCodeInput = mDialog.findViewById(R.id.pPopupBuinessCodeInput);
            String mBusinessCode = mPopupBuinessCodeInput.getText().toString();

            boolean mStatus = (boolean)mEvent.invokeObserver(Collections.singletonList(mBusinessCode), M_BUSINESS_CODE_SUBMIT);
            if(!mStatus){
                mPopupBuinessCodeError.setText(R.string.ALERT_BUSINESS_INFO_INPUT_ERROR);
            }else {
                mDialog.dismiss();
            }
        }
    ...
    public void onTrigger(List<Object> pData, pluginEnums.eMessageManager pEventType) {
        ...
        if (!pEventType.equals(M_NOT_SUPPORTED)) {
            onClearReference();
            mData = null;
        }
